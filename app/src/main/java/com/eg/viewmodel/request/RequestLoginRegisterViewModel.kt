package com.eg.viewmodel.request

import androidx.lifecycle.MutableLiveData
import com.eg.app.network.apiService
import com.eg.data.model.bean.LoginBean
import com.eg.data.repository.request.HttpRequestCoroutine
import com.eg.data.repository.request.UserInfo
...
//    var loginResult2 = MutableLiveData<ResultState<ApiResponse<UserInfo>>>()
    ...
    fun loginReq(time: String, username: String, password: String, otp: String) {
        //1.这种是在 Activity/Fragment的监听回调中拿到已脱壳的数据（项目有基类的可以用）
        request(
                { apiService.login(time, username, password, otp) }//请求体
...
//        request({ HttpRequestCoroutine.login(username, password, otp) }, {
